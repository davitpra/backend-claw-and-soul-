...
@Injectable()
export class StyleDrivenPromptStrategy extends BaseStyleStrategy {
  readonly key = 'style-driven-prompt';
  private readonly logger = new Logger(StyleDrivenPromptStrategy.name);
  ...
  async execute(ctx: PipelineContext): Promise<PipelineResult> {
    ...
    if (!visionConfig) {
      throw new Error(
        `Style "${style.name}" requires a visionConfig for strategy "style-driven-prompt"`,
      );
    }
    if (!imageGenConfig) {
      throw new Error(
        `Style "${style.name}" requires an imageGenConfig for strategy "style-driven-prompt"`,
      );
    }
