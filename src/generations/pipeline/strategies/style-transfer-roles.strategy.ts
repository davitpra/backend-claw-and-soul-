...
@Injectable()
export class StyleTransferRolesStrategy extends BaseStyleStrategy {
  readonly key = 'style-transfer-roles';
  private readonly logger = new Logger(StyleTransferRolesStrategy.name);
  ...
  async execute(ctx: PipelineContext): Promise<PipelineResult> {
    ...
    if (!imageGenConfig) {
      throw new Error(
        `Style "${style.name}" requires an imageGenConfig for strategy "style-transfer-roles"`,
      );
    }

    if (!style.styleReferenceUrl) {
      throw new Error(
        `Style "${style.name}" requires styleReferenceUrl set for strategy "style-transfer-roles"`,
      );
    }
    ...
    const falResult = await this.fal.generate({
      model: falModel,
      prompt,
      imageUrls: [styleReferenceUrl, ctx.petPhotoUrl],
      aspectRatio,
