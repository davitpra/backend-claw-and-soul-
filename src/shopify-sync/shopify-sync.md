...
Implements a 3-layer synchronization strategy that keeps `product_references` in sync with the Shopify product catalog. Shopify remains the source of truth for all commercial data; this module only mirrors the minimum fields needed for the compatibility matrix (`style_format_product_compat`) to function internally.
...
All three layers share the same upsert logic via `ProductSyncService`.

## Files Structure

- `shopify-sync/product-sync.service.ts`: Shared upsert and soft-delete logic used by all three layers.
...
| :----- | :---------------------------------------------- | :------------------------------------------------- |
| `POST` | `/api/webhooks/shopify/product/create`          | Shopify `products/create` — enqueues upsert job    |
| `POST` | `/api/webhooks/shopify/product/update`          | Shopify `products/update` — enqueues upsert job    |
...
- **Soft Delete + Cascade**: Deactivating a product sets `isActive = false` on `product_references` and also deactivates all related rows in `style_format_product_compat` within a single `$transaction`.
- **Cron Reconciliation**: `@Cron('0 */6 * * *')` fetches all Shopify products, upserts each one, then soft-deletes any `product_references` that are no longer present in Shopify.
...
- **Sync Logging**: Every sync run is recorded in `sync_logs` with counters (`productsChecked`, `productsCreated`, `productsUpdated`, `productsDeactivated`) and error details.
...
- **Product deactivated**: soft-deleted in `product_references` and all related `style_format_product_compat` rows set to `isActive = false`. Existing `generations` referencing the product are not affected.
